...
public class Flee : MonoBehaviour
{
    ...
    private Vector3 origin = new Vector3(-2.71f, -0.85f, 13.38f);
    void Start()
    {
        start = transform.position;
        behs = new Behavior[] { RayCastAvoid, CollisionPrediction, ConeCheck };
        target = 1.5f * transform.position + 0.5f * origin - chaser.transform.position;
    ...
    {
        lastTarget = transform.position + origin - chaser.transform.position;
        i++;
        if (i == 3)
        {
            i = 0;
        ...
        if (cols.Count != 0)
        {
            ...
            if (count != -1)
            {
                ...
                if (target.magnitude <= 1f || t < 0.6f)
                {
                    ...
                    Vector3 tmpTarget = back + transform.position + coneEvadeMulti * distanceOffset * (1.5f * transform.position + 0.5f * origin - chaser.transform.position).normalized + (coneEvadeMulti * strength * transform.right) / distanceOffset;
                ...
                else
                {
                    ...
                    {
                        lastTarget = 1.5f * transform.position + 0.5f * origin - chaser.transform.position;
                        target = 1.5f * transform.position + 0.5f * origin - chaser.transform.position;
                    }
                    Pursue(target);
                }
